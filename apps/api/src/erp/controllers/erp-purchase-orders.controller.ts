...
@ApiTags('erp — pedidos de compra')
@Controller('erp/purchase-orders')
@UseGuards(JwtAuthGuard, ErpBusinessGuard)
@ApiBearerAuth()
@ApiHeader({ name: 'X-Business-Id', required: true })
export class ErpPurchaseOrdersController {
  constructor(private readonly svc: ErpPurchaseOrderService) {}
  ...
  @Post()
  @ApiOperation({ summary: 'Criar pedido com fornecedor e itens' })
  create(
    @SelectedBusiness() business: ErpBusiness,
  ...
  patchStatus(
    @SelectedBusiness() business: ErpBusiness,
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: PatchPurchaseOrderStatusDto,
  ) {
