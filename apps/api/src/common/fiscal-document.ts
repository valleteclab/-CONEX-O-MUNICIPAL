...
export function normalizeFiscalDocument(
...
export function supportsCurrentCnpjLookup(
  value: string | null | undefined,
): boolean {
  return parseFiscalDocument(value).kind === 'cnpj_numeric';
}

export function supportsCurrentPlugNotasDocument(
  value: string | null | undefined,
): boolean {
  const parsed = parseFiscalDocument(value);
  return parsed.kind === 'cpf' || parsed.kind === 'cnpj_numeric';
}
...
function isValidCnpj(cnpj: string): boolean {
  ...
  const firstDigit = calculateCnpjCheckDigit(base, CNPJ_WEIGHTS_FIRST);
...
function calculateCnpjCheckDigit(base: string, weights: number[]): number {
  const sum = base.split('').reduce((acc, char, index) => {
    return acc + getCnpjCharValue(char) * weights[index];
  }, 0);
